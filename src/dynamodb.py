...
def get_dynamodb_autoscaling_settings(base_table_name, region, index_name=None ):
    ...
    response = app_autoscaling.describe_scalable_targets(ResourceIds=[resource_id], ServiceNamespace='dynamodb')
    ...
    for base_table_name in base_table_names:
        ...
        try:
            ...
            if len(response['ScalableTargets']) == 0:
                continue
            settings_list.append(get_dynamodb_autoscaling_settings(base_table_name=base_table_name,region=region))
        ...
        for index in global_indexes:
            ...
            try:
                ...
                if len(response['ScalableTargets']) == 0:
                    continue
                settings_list.append(get_dynamodb_autoscaling_settings(base_table_name=base_table_name,region=region, index_name = index_name))
            except app_autoscaling.exceptions.ValidationException as e:
                if 'does not exist' in e.response['Error']['Message']:
                    continue
                else:
...
def get_dynamodb_scaling_info(params):
    ...
    for region in regions:
        ...
        all_settings_df = get_all_dynamodb_autoscaling_settings_with_indexes(region)
    ...
    if len(all_autoscaling_info) > 0: 
        all_autoscaling_info = pd.concat(all_autoscaling_info,ignore_index= True)
        all_autoscaling_info['index_name'] = all_autoscaling_info.apply(lambda x: x['base_table_name'] if pd.isnull(x['index_name']) else x['base_table_name'] + ':' + x['index_name'], axis=1)
        all_autoscaling_info['metric_name'] = all_autoscaling_info['metric_name'].replace({'dynamodb:table:ReadCapacityUnits': 'ProvisionedReadCapacityUnits', 'dynamodb:index:ReadCapacityUnits': 'ProvisionedReadCapacityUnits'}, regex=True)
        all_autoscaling_info['metric_name'] = all_autoscaling_info['metric_name'].replace({'dynamodb:table:WriteCapacityUnits': 'ProvisionedWriteCapacityUnits', 'dynamodb:index:WriteCapacityUnits': 'ProvisionedWriteCapacityUnits'}, regex=True)
    else:
    ...
    if all_autoscaling_info is not None: 
        ...
        write_to_s3(all_autoscaling_info, location_estimate, 'overwrite', athena_tablename+'_dynamodb_info')
