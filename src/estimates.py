...
def estimateUnits(read, write,readutilization,writeutilization):
    ...
    #columns [metric_name,accountid,timestamp,name,units,unitps,estunit]
    finalreadcu = []
    count = 0
    finalwritecu = []
    prevread = read[0]
    ...
    prevread[6] = (prevread[5] / readutilization) * 100
    prevwrite[6] = (prevwrite[5] / writeutilization) * 100
    for i in range(1, len(read)):
        currentread = read[i]
        currentwrite = write[i]
        
        date_time_obj = currentread[2].to_pydatetime()
        ...
        if i <= 2:
            currentread[6] = prevread[6]
            currentwrite[6] = prevwrite[6]
        ...
        last2read = [v[5] for v in list(read[i - 2: i])]
        last2write = [v[5] for v in list(write[i - 2: i])]
        ...
        if currentread[0] == 'ConsumedReadCapacityUnits':
            if maxVread == (last2minread / readutilization) * 100:
                ...
                currentread[6] = (last2maxread / readutilization) * 100
                

            else:

                currentread[6] = maxVread

        if currentwrite[0] == 'ConsumedWriteCapacityUnits':
            if maxVwrite == (last2minwrite / writeutilization) * 100:

                currentwrite[6] = (last2maxwrite / writeutilization) * 100
            else:

                currentwrite[6] = maxVwrite
        ...
        last15read = [v[5] for v in list(read[i - 15: i])]
        last15read2 = [v[6] for v in list(read[i - 15: i])]
        # print(last15)
        last15Maxread = max(last15read)
        # Create list from last 15 Consumed Units
        last15write = [v[5] for v in list(write[i - 15: i])]
        last15write2 = [v[6] for v in list(write[i - 15: i])]
        # print(last15)
        last15Maxwrite = max(last15write)
        if count < 4:
            if not decrease15(last15read2):
                currentread[6] = minA((last15Maxread / readutilization) * 100, currentread[6])
                if prevread[6] > currentread[6]:

                    count += 1                   

            if not decrease15(last15write2):
                currentwrite[6] = minA((last15Maxwrite / writeutilization) * 100, currentwrite[6])
                if prevwrite[6] > currentwrite[6]:
                    count += 1
            ...
            if i >= 60:
                ...
                last60read = [v[6] for v in list(read[i - 60: i])]
                last60write = [v[6] for v in list(write[i - 60: i])]
                ...
                if not decrease60(last60write) and  not decrease60(last60read):
                    ...
                    if prevread[6] > currentread[6] or prevwrite[6] > currentwrite[6] :
...
def estimate(df,readutilization,writeutilization):
    ...
    if len(finalcu) > 0:
        ...
        finaldf.columns = ['metric_name', 'accountid', 'timestamp',
