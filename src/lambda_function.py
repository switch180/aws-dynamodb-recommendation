...
def get_metrics(params, regions):
    print("getting CloudWatch Metrics for " +
          params['dynamodb_tablename'] + " Dynamo Tables")
...
def recommendation(params):
    recommendation_status = athenaquery.create_dynamo_mode_recommendation(
        params)
...
def autoscaling_recommendation(params):
    autoscaling_status = athenaquery.create_dynamo_autoscaling_recommendation(
        params)
        ...
        if key in event:
            ...
            if key == 'action':
                ...
                if not isinstance(value, str) or not value.strip() or value not in valid_actions:
                    raise ValueError(
                        f"Invalid action '{key}'. action must be a non-empty string and one of {valid_actions}")
            elif key == 'accountid':
                if not isinstance(value, str) or not value.isalnum():
                    raise ValueError(
                        f"Invalid accountid '{key}'. accountid must be a non-empty string")
            elif key == 'dynamodb_tablename':
                if not isinstance(value, str) or not value.isalnum():
                    raise ValueError(
                        f"Invalid dynamodb_tablename '{key}'. dynamodb_tablename must be a non-empty string")
            elif key == 'dynamodb_read_utilization':
                if not isinstance(value, int) or value < 0 or value > 100:
                    raise ValueError(
                        f"Invalid dynamodb_read_utilization '{key}'. dynamodb_read_utilization must be an integer between 0 and 100")
                ...
                if not isinstance(value, int) or value < 0:
                    raise ValueError(
                        f"Invalid number_of_days_look_back '{key}'. number_of_days_look_back must be a positive integer")
                ...
                if not isinstance(value, str) or not re.match(r'20[0-9]{2}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}', value):
                    raise ValueError(
                        "cloudwatch_metric_end_datatime must be a valid string in format 'YYYY-MM-DD HH:MM:SS'")
            elif key == 'regions':
                ...
                for region in value:
                    if not isinstance(region, str) or not region.strip():
                        raise ValueError(
                            f"each element of {key} must be a valid alphanumeric string")
            params[key] = value
    return params
